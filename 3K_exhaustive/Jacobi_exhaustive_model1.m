...
%%   3^2 = 9 representations of the Poisson matrix J for Model 1
%%   (K=2, M=4, dense coupling, gyrostat 1 on {x1,x2,x3}, gyrostat 2
%%   on {x2,x3,x4}). 
%%
%% PAPER REFERENCE
...
%% Find zero values
izero = find(Jacobiijkmlist == sym(0));

for i = 1:numel(izero)

    dlisti = dlist(izero(i),:);

    J1 = squeeze(J1all(:,:,dlisti(1)));
    J2 = squeeze(J2all(:,:,dlisti(2)));
